...
heartbeat_reconnect() {
	...
	if [[ $? -ne 0 ]]; then
	    echo "Packet timed out, reconnecting"
            reconnect
	    if [[ $? -ne 0 ]]; then
		echo "Falling back to connecting to default network $DEFAULT"
		connect $DEFAULT
	    fi
	else
